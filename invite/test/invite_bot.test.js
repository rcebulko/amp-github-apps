...
describe('Invite Bot', () => {
  ...
  beforeEach(() => {
    inviteBot = new InviteBot(/*client=*/null, 'test_repo', new Datastore());

    jest.spyOn(github, 'inviteUser');
    jest.spyOn(github, 'addComment');
  ...
  describe('tryInvite', () => {
    ...
    describe('when the user has a pending invite from the bot', () => {
      ...
      it('it does not attempt to send an invite', async done => {
        ...
        expect(github.inviteUser).not.toBeCalled();
        done();
      });
      ...
      it('comments that there is already an invite pending', async done => {
        await inviteBot.tryInvite(newInvite);
        expect(github.addComment).toBeCalledWith(
          'test_repo',
          1337,
    ...
    describe('when the user is a member', () => {
      beforeEach(() => {
        github.inviteUser.mockImplementation(() => false);
      });

      it('attempts to send an invite', async done => {
        await inviteBot.tryInvite(newInvite);
        expect(github.inviteUser).toBeCalledWith('someone');
        done();
      });
    ...
    describe('when the user is not a member', () => {
      beforeEach(() => {
        github.inviteUser.mockImplementation(() => true);
      });
    ...
    describe('when the invite fails', () => {
      beforeEach(() => {
        github.inviteUser.mockImplementation(() => {
      ...
      it('logs an error', async done => {
        try {
          await inviteBot.tryInvite(invite);
        } catch (e) {}
  ...
  describe('tryAssign', () => {
    it('assigns the user to the issue', async done => {
      ...
      expect(github.assignIssue).toBeCalledWith('test_repo', 1337, 'someone');
    });
